...
async function fetchXhsViaTikHub(noteUrl) {
  ...
  try {
    ...
    const noteData = json?.data?.note_item?.note_card ?? json?.data?.note_card ?? json?.data?.items?.[0]?.note_card ?? json?.data;
    if (!noteData) {
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-BLwxRYOU.js").then((n) => n.a2).then(
      (m) => m.default ?? m
    );
  }
