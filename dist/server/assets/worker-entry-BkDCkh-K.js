...
async function fetchXhsViaTikHub(noteUrl) {
  ...
  try {
    ...
    const params = new URLSearchParams({ note_id: ref2.noteId });
    if (ref2.xsecToken) params.set("xsec_token", ref2.xsecToken);
    console.log(`[XHS] Fetching via TikHub API, note ID: ${ref2.noteId}, has xsec_token: ${!!ref2.xsecToken}`);
    ...
    const noteData = json?.data?.note_item?.note_card ?? json?.data?.note_card ?? json?.data?.note_items?.[0]?.note_card ?? json?.data?.items?.[0]?.note_card ?? json?.data;
    if (!noteData) {
    ...
    const tags = (noteData.tag_list || noteData.tags || []).map((t) => t?.name || t).filter(Boolean);
    const parts = [
      title2 && `标题: ${title2}`,
      desc && `内容: ${desc}`,
    ...
    if (result.length > 20) {
      console.log(`[XHS] TikHub extracted: "${title2}" (${desc.length} chars)`);
      return result;
    }
    return null;
  } catch (err) {
    console.error(`[XHS] TikHub fetch failed:`, err.message);
    return null;
  }
}
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-DcVOWMmA.js").then((n) => n.a2).then(
      (m) => m.default ?? m
    );
  }
