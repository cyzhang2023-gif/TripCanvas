...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-DFlOycZ2.js").then((n) => n.a2).then(
      (m) => m.default ?? m
    );
  }
...
function getRouteCoverUrl(city) {
...
async function handleApiRequest(request, env2) {
    ...
    try {
      ...
      if (pool) {
        ...
        n8nExplore = n8nRes.rows.map((r) => ({
          ...
          cover: r.cover_url || getRouteCoverUrl(r.city || ""),
  ...
  if (pathname === "/api/n8n-routes" && request.method === "GET") {
    ...
    const rows = result.rows.map((r) => ({
      ...r,
      cover_url: r.cover_url || getRouteCoverUrl(r.city || "")
    }));
    return apiJson(rows);
  }
